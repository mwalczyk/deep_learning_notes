...
* Two random variables $$x$$ and $$y$$ are **conditionally independent** given a random variable $$z$$ if the conditional probability distribution over $$x$$ and $$y$$ factorizes in this way for every value of $$z$$: $$p(x = \textit{x}, y = \textit{y}\mid z = \textit{z}) = p(x = \textit{x}\mid z = \textit{z})p(y = \textit{y}\mid z = \textit{z})$$.

...
* We can generalize the chain rule beyond the scalar case. Suppose that $$x\in R^{m}$$ and $$y\in R^{n}$$. Let $$g$$ be an intermediate function that maps from $$R^{m}$$ to $$R^{n}$$. Finally, let $$f$$ be a function that maps from $$R^{n}$$ to $$R$$. The gradient of $$z$$ with respect to $$x$$ can be written as the product of a Jacobian matrix $$\frac{dy}{dx}$$ and a gradient $$\bigtriangledown\_{y}z$$. The back-propagation algorithm consists of performing such a Jacobian-gradient product for each operation in the graph.
